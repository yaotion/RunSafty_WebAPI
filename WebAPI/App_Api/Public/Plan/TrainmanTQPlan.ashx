<%@ WebHandler Language="C#"  Class="TF.RunSaftyAPI.App_Api.Public.Plan.TrainmanTQPlan" %>
using System;
...
namespace TF.RunSaftyAPI.App_Api.Public.Plan
{
    ...
    public class TrainmanTQPlan : IHttpHandler
    {
        string sRequest = "";
        ...
        /// </summary>
        /// <returns></returns>
        public string gotoTrainmanTQPlan()
        {
            //获取
        ...
        + "\",\"trainJiaoluName\":\"" + string.Empty + "\",\"trainJiaoluGUID\":\"" + string.Empty + "\",\"trainmanList\":[");
            ...
            //获取退勤信息
            string strSqlPlan = @"select top 1 * from VIEW_Plan_Trainman
     where strTrainJiaoluGUID in (select strTrainJiaoluGUID from TAB_Base_TrainJiaoluInSite where strSiteGUID = @SiteGUID) and 
     nPlanState = 7 AND (strTrainmanGUID1 = @TrainmanGUID or strTrainmanGUID2 =@TrainmanGUID or strTrainmanGUID3 = @TrainmanGUID or strTrainmanGUID4 =@TrainmanGUID) order by dtStartTime desc";
        ...
        {
            string strHelp = @"1.6	获取乘务员退勤计划信息
 调用参数
     参数格式
        cid=xxx&tid=
