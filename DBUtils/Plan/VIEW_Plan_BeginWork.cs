/**  版本信息模板在安装目录下，可自行修改。
* VIEW_Plan_BeginWork.cs
*
* 功 能： N/A
* 类 名： VIEW_Plan_BeginWork
*
* Ver    变更日期             负责人  变更内容
* ───────────────────────────────────
* V0.01  2014/9/24 17:04:54   N/A    初版
*
...
{
	/// <summary>
	/// 数据访问类:VIEW_Plan_BeginWork
	/// </summary>
	public partial class VIEW_Plan_BeginWork
	{
		public VIEW_Plan_BeginWork()
		{}
		#region  BasicMethod
		...
		public TF.RunSafty.Model.VIEW_Plan_BeginWork GetModel(int nid)
		{
			...
			TF.RunSafty.Model.VIEW_Plan_BeginWork model=new TF.RunSafty.Model.VIEW_Plan_BeginWork();
        ...
        /// 得到一个对象实体
        /// </summary>
        public TF.RunSafty.Model.VIEW_Plan_BeginWork DataRowToModel(DataRow row)
        {
            TF.RunSafty.Model.VIEW_Plan_BeginWork model = new TF.RunSafty.Model.VIEW_Plan_BeginWork();
            if (row != null)
            {
                if (row.Table.Columns.Contains("nFlowState"))
                {                    
                    if (row["nFlowState"] == DBNull.Value)
                    {
                        model.nFlowState = 0;
                    }
                    else
                    {
                        model.nFlowState = Convert.ToInt32(row["nFlowState"]);
                    }
                    
                }
                if (row["dtBeginworkTime"] != null)
                {
                    DateTime dtBeginworkTime;
                    if (DateTime.TryParse(row["dtBeginworkTime"].ToString(), out dtBeginworkTime))
                    {
                        model.dtBeginWorkTime = dtBeginworkTime;
                    }
                }
                ...
                {
                    //model.dtTrainmanCreateTime = DateTime.Parse(row["dtTrainmanCreateTime"].ToString());
                ...
                if (row["strBeginWorkGUID1"] != null)
                {
                    model.strBeginWorkGUID1 = row["strBeginWorkGUID1"].ToString();
                }
                if (row["strBeginWorkGUID2"] != null)
                {
                    model.strBeginWorkGUID2 = row["strBeginWorkGUID2"].ToString();
                }
                if (row["strBeginWorkGUID3"] != null)
                {
                    model.strBeginWorkGUID3 = row["strBeginWorkGUID3"].ToString();
                }
                if (row["strBeginWorkGUID4"] != null)
                {
                    model.strBeginWorkGUID4 = row["strBeginWorkGUID4"].ToString();
                }
                ...
                if (row["strICCheckResult"] != null)
                {
                    model.strICCheckResult = row["strICCheckResult"].ToString();
                }
            }
            return model;
        }
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select dtBeginworkTime,nFlowState,strTrainJiaoluName,strTrainmanTypeName,strTrainPlanGUID,strTrainJiaoluGUID,strTrainNo,strTrainNumber,dtStartTime,dtChuQinTime,strStartStation,strEndStation,dtCreateTime,nPlanState,strTrainTypeName,strStartStationName,strEndStationName,nTrainmanTypeID,SendPlan,nNeedRest,dtArriveTime,dtCallTime,nKehuoID,dtRealStartTime,nPlanType,nDragType,nRemarkType,strRemark,strCreateSiteGUID,strCreateUserGUID,strCreateUserID,strCreateUserName,strCreateSiteName,strTrainmanNumber1,strTrainmanName1,nPostID1,strWorkShopGUID1,strTelNumber1,dtLastEndWorkTime1,strTrainmanGUID1,strTrainmanGUID2,strTrainmanNumber2,strTrainmanName2,strWorkShopGUID2,strTelNumber2,strTrainmanNumber3,strTrainmanGUID3,dtLastEndWorkTime2,nPostID2,strTrainmanName3,nPostID3,strWorkShopGUID3,strTelNumber3,dtLastEndWorkTime3,strDutyGUID,strGroupGUID,strDutySiteGUID,dtTrainmanCreateTime,dtFirstStartTime,nid,nDragTypeName,strKehuoName,nTrainmanState1,nTrainmanState2,nTrainmanState3,strBak1,dtLastArriveTime,strMainPlanGUID,strTrainmanName4,strTrainmanNumber4,nPostID4,strTelNumber4,dtLastEndWorkTime4,nTrainmanState,strTrainmanGUID4,strRemarkTypeName,strPlanTypeName,strPlaceID,strStateName,nDriverType1,nDriverType2,nDriverType3,nDriverType4,strABCD1,strABCD2,strABCD3,strABCD4,isKey1,isKey2,isKey3,isKey4,strPlanStateName,'' as strStationName,'' as strStationNumber,strPlaceName,'' as strStationGUID,dtTestTime1,nVerifyID1,DrinkImage1,nDrinkResult1,dtTestTime2,nVerifyID2,DrinkImage2,nDrinkResult2,dtTestTime3,nVerifyID3,DrinkImage3,nDrinkResult3,strBeginWorkGUID1,strBeginWorkGUID2,strBeginWorkGUID3,strBeginWorkGUID4,dtTestTime4,nVerifyID4,DrinkImage4,nDrinkResult4,strICCheckResult ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM VIEW_Plan_BeginWork ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from VIEW_Plan_BeginWork T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "VIEW_Plan_BeginWork";
