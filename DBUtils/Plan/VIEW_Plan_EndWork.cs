...
namespace TF.RunSafty.DAL
{
	...
	public partial class VIEW_Plan_EndWork
	{
        ...
        /// 得到一个对象实体
        /// </summary>
        public TF.RunSafty.Model.VIEW_Plan_EndWork DataRowToModel(DataRow row)
        {
            TF.RunSafty.Model.VIEW_Plan_EndWork model = new TF.RunSafty.Model.VIEW_Plan_EndWork();
            if (row != null)
            {
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select strTrainJiaoluName,strTrainmanTypeName,strTrainPlanGUID,strTrainJiaoluGUID,strTrainNo,strTrainNumber,dtStartTime,dtChuQinTime,strStartStation,strEndStation,dtCreateTime,nPlanState,strTrainTypeName,strStartStationName,strEndStationName,nTrainmanTypeID,SendPlan,nNeedRest,dtArriveTime,dtCallTime,nKehuoID,dtRealStartTime,nPlanType,nDragType,nRemarkType,strRemark,strCreateSiteGUID,strCreateUserGUID,strCreateUserID,strCreateUserName,strCreateSiteName,strTrainmanNumber1,strTrainmanName1,nPostID1,strWorkShopGUID1,strTelNumber1,dtLastEndWorkTime1,strTrainmanGUID1,strTrainmanGUID2,strTrainmanNumber2,strTrainmanName2,strWorkShopGUID2,strTelNumber2,strTrainmanNumber3,strTrainmanGUID3,dtLastEndWorkTime2,nPostID2,strTrainmanName3,nPostID3,strWorkShopGUID3,strTelNumber3,dtLastEndWorkTime3,strDutyGUID,strGroupGUID,strDutySiteGUID,dtTrainmanCreateTime,dtFirstStartTime,nid,nDragTypeName,strKehuoName,nTrainmanState1,nTrainmanState2,nTrainmanState3,strBak1,dtLastArriveTime,strMainPlanGUID,strTrainmanName4,strTrainmanNumber4,nPostID4,strTelNumber4,dtLastEndWorkTime4,nTrainmanState,strTrainmanGUID4,strRemarkTypeName,strPlanTypeName,strPlaceID,strStateName,nDriverType1,nDriverType2,nDriverType3,nDriverType4,strABCD1,strABCD2,strABCD3,strABCD4,isKey1,isKey2,isKey3,isKey4,strPlanStateName,'' as strStationName,'' as strStationNumber,'' as strPlaceName,'' as strStationGUID,dtTestTime1,nVerifyID1,nDrinkResult1,dtTestTime2,nVerifyID2,nDrinkResult2,dtTestTime3,nVerifyID3,nDrinkResult3,DrinkImage3,DrinkImage1,DrinkImage2,strEndWorkGUID1,strEndWorkGUID2,strEndWorkGUID3,strEndWorkGUID4,dtTestTime4,nVerifyID4,DrinkImage4,nDrinkResult4 ");
            strSql.Append(" FROM VIEW_Plan_EndWork ");
			if(strWhere.Trim()!="")
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM VIEW_Plan_EndWork ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from VIEW_Plan_EndWork T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "VIEW_Plan_EndWork";
        ...
        public DataSet GetEndWorkPlans(string strBeginTime, string strEndTime, string strTrainjiaolus)
        {
            string strSql = string.Format(@"
select *,b.dtTurnStartTime ,bSigned,bIsOver,c.nTurnMinutes,c.nTurnAlarmMinutes from VIEW_Plan_EndWork 
left join VIEW_WorkTime_Turn_Branch_MaxStartTime as b on   VIEW_Plan_EndWork.strTrainPlanGUID = b.strFlowID  
left join TAB_Base_TrainmanTypeTurn as c 
on VIEW_Plan_EndWork.nTrainmanTypeID=c.nTrainmanTypeID and VIEW_Plan_EndWork.nKeHuoID=c.nKeHuoID   
where 
 (dtLastArriveTime >='{0}' and 
  strTrainJiaoluGUID in ({2})) 
    or
  (nPlanState = 7 and  bIsOver = 0 and '{1}' > DATEADD (mi,nTurnAlarmMinutes,dtTurnStartTime) and 
    strTrainJiaoluGUID in ({2})) order by nPlanState,dtLastArriveTime desc
", strBeginTime,strEndTime,strTrainjiaolus);
            DataSet set = SqlHelper.ExecuteDataset(SqlHelper.ConnString, CommandType.Text, strSql);
            return set;
        }
