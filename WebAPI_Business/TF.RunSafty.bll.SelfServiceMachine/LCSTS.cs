...
namespace TF.RunSafty.SelfServiceMachine
{
    ...
    public class SpecificOnDuty_ts
    {
        ...
        public LCSpecificOnDuty.JsonModel TangShanSpecific(string strgh, string strxcjl, string strcid, string strcx, string strch, string strcc, LCSpecificOnDuty.PSpecificOnduty paramModel)
        {
            DBSpecificOnDuty ea = new DBSpecificOnDuty();

            strgh = "";//工号字符串 

            foreach (LCSpecificOnDuty.gh g in paramModel.gh)
            {
                if (g.ghID.Length == 7)
                {
            ...
            //查询完毕后渲染html
            string html = TF.CommonUtility.CommonHelper.RenderHtml("Tstop.htm", "");//头部的html代码
            ...
            {
                DataTable dtRenYuan = ea.GetRenYuanByExcel(strgh.Split(',')[sp]);
                string strRenYuan1 = "";
                string strRenYuan2 = "";
                string strRenYuan3 = "";
                string strRenYuan4 = "";
                ...
                strRenYuan += "<tr><td>" + TF.CommonUtility.CommonHelper.RenderHtml("TsTab_Test.htm", AllTasts(strgh.Split(',')[sp])) + "</td></tr>";
                strRenYuan += "<tr><td>" + TF.CommonUtility.CommonHelper.RenderHtml("TsTab_SpecificOnDuty_LiangWei.htm", ea.GetAllLiangWei(strgh.Split(',')[sp]).Rows) + "</td></tr>";
            ...
            string strLinSuiXiu = "<tr><td>" + TF.CommonUtility.CommonHelper.RenderHtml("TsTab_SpecificOnDuty_LingSuiXiu.htm", AllLingSuiXiu(strcx, strch)) + "</td></tr>";
            ...
            //生成EXCEL
            string strExcelHtml = TF.CommonUtility.CommonHelper.RenderHtml("TsExcel.htm", "");
            ...
            LCSpecificOnDuty.JsonModel jsonModel = new LCSpecificOnDuty.JsonModel();
            jsonModel.result = "0";
            jsonModel.resultStr = "返回成功";
            Random r = new Random();
            jsonModel.data = new LCSpecificOnDuty.ResultData();
